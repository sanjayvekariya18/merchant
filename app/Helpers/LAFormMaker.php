...
class LAFormMaker
{
	...
	public static function input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
	{
		...
		switch ($field_type->name) {
			...
			case 'Date':
				...
				$params["class"] = "form-control mydatepicker";
			...
			case 'Datetime':
				...
				$params["class"] = "form-control mydatetimepicker";
			...
			case 'AutoComplete':
				...
				if($popup_vals != "") {
					$popup_vals = LAFormMaker::process_values($popup_vals,$popup_field_id,$popup_field_name);
				} else {
					$popup_vals = array();
				}
			...
			case 'File':
				...
				if(!is_numeric($default_val)) {
					$default_val = self::INIT_VALUE;
				}
				$out .= Form::hidden($field_name, $default_val, $params);

				if($default_val != self::INIT_VALUE) {
					$upload = \App\Upload::find($default_val);
				}
				if(isset($upload->id)) {
					$upload_path = explode("public\\",$upload->path)[self::FIRST_VALUE];						
					if(in_array($upload->extension, ["jpg", "png", "gif", "jpeg"])) {
						$fileImage = "<img src='".url($upload_path)."' width='100px'>";
					} else {
					...
					$out .= "<a class='btn btn-default btn_upload_file hide' file_type='file' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
						<a class='uploaded_file' target='_blank' href='".url($upload_path)."'>".$fileImage."</i><i title='Remove File' class='fa fa-times'></i></a>";
				} else {
			...
			case 'Files':
				...
				if(is_array($default_val_arr) && count($default_val_arr) > self::INIT_VALUE) {
					$uploadIds = array();
					$uploadImages = "";
					foreach ($default_val_arr as $uploadId) {
						...
						if(isset($upload->id)) {
							...
							$uploadImages .= "<a class='uploaded_file2' upload_id='".$upload->id."' target='_blank' href='".url($upload_path)."'>".$fileImage."<i title='Remove File' class='fa fa-times'></i></a>";
						}
					}
					
					$out .= Form::hidden($field_name, json_encode($uploadIds), $params);
					if(count($uploadIds) > self::INIT_VALUE) {
			...
			case 'Image':
				...
				if(isset($upload->id)) {
					...
					$out .= "<a class='btn btn-default btn_upload_image hide' file_type='image' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
						<div class='uploaded_image'><img src='".url($upload_path."?s=150")."'><i title='Remove Image' class='fa fa-times'></i></div>";
				} else {
			...
			case 'Taginput':
				...
				$default_val = LAFormMaker::process_values($default_val,$popup_field_id,$popup_field_name);
	...
	**/
	...
	public static function process_values($json,$field_value="id",$field_text="name") {
		$out = array();
		...
		if(is_string($json) && starts_with($json, "@")) {
			...
			if(isset($module->id)) {
				...
				if (Schema::hasTable($table_name)) {
					...
					if(isset($result[self::INIT_VALUE])) {
						$view_col_test_1 = $field_text;
						if(isset($result[self::INIT_VALUE]->$view_col_test_1)) {
							// Check whether view column name == "name"
							$view_col = $view_col_test_1;
						} else {
							// retrieve the second column name which comes after "id"
							$arr2 = $result[self::INIT_VALUE]->toArray();
							$arr2 = array_keys($arr2);
							$view_col = $arr2[self::FIRST_VALUE];
							// if second column not exists
							if(!isset($result[self::INIT_VALUE]->$view_col)) {
								$view_col = "";
							}
						}
						...
						if($view_col != "") {
							...
							foreach ($result as $row) {
								$out[$row->$field_value] = $row->$view_col;
							}							
						} else {
	...
	**/
	public static function display($module, $field_name, $class = 'form-control')
	{
		...
		switch ($field_type->name) {
			...
			case 'Dropdown':
			case 'AutoComplete':
				$values = LAFormMaker::process_values($fieldObj['popup_vals'],$fieldObj['popup_field_id'],$fieldObj['popup_field_name']);
				if(starts_with($fieldObj['popup_vals'], "@")) {
					if($value != self::INIT_VALUE) {
						if(isset($moduleVal->id)) {
							$value = "<span class='label label-primary' style='margin-left:5px;'>".$values[$value]."</span>";
						} else {
							$value = "<span class='label label-primary' style='margin-left:5px;'>".$values[$value]."</span>";
						}
					} else {
			...
			case 'File':
				if($value != self::INIT_VALUE) {
					...
					if(isset($upload->id)) {
						$upload_path = explode("public\\",$upload->path)[self::FIRST_VALUE];
						$value = '<a class="preview" target="_blank" href="'.url($upload_path).'">
				...
				if($value != "" && $value != "[]" && $value != "null" && starts_with($value, "[")) {
					...
					foreach ($uploads as $uploadId) {
						...
						if(isset($upload->id)) {
							...
							$uploads_html .= '<a class="preview" target="_blank" href="'.url($upload_path).'" data-toggle="tooltip" data-placement="top" data-container="body" style="display:inline-block;margin-right:5px;" title="'.$upload->name.'">
									'.$fileImage.'</a>';
						}
					}
					$value = $uploads_html;
			...
			case 'Image':
				...
				if(isset($upload->id)) {
					$upload_path = explode("public\\",$upload->path)[self::FIRST_VALUE];
					$value = '<a class="preview" target="_blank" href="'.url($upload_path).'"><img src="'.url($upload_path."?s=150").'" width="100px"></a>';
				} else {
					...
					if(starts_with($fieldObj['popup_vals'], "@")) {
						...
						foreach ($values as $key => $val) {
							if(in_array($key, $valueSel)) {
								$valueOut .= "<span class='label label-primary' style='margin-left:5px;'>".$val."</span>";
							}
						}
					} else {
						$valueSel = json_decode($value);
	...
	**/
	...
	{
		if(count($fields) == self::INIT_VALUE) {
	...
	**/
	public static function la_access($module_id, $access_type = "access", $user_id = self::INIT_VALUE)
	{
