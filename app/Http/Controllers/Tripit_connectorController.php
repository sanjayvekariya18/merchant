...
class Tripit_connectorController extends Controller
{
    ...
    public function __construct(){
        ...
        if(strcmp($connectionStatus['type'],"error")==0){
	        ...
	        if(strcmp($result['type'],'error')==0){
	        	...
	        	$this->api_key = Config::get('services')[strtolower($this->connectorName)]['client_id'];
	        	$this->api_secret_key = Config::get('services')[strtolower($this->connectorName)]['client_secret'];
	        	$this->callback_url = Config::get('services')[strtolower($this->connectorName)]['redirect'];
	        }
        }
    }
    /**
    ...
    {
        try{
        // Request for get request token using api key and secret key

            $oauth_credential = new OAuthConsumerCredential($this->api_key,$this->api_secret_key);
            ...
            return Redirect::to('https://www.tripit.com/oauth/signIn?oauth_token='.$request_token['oauth_token'].'&oauth_callback='.$this->callback_url.'&is_sign_in=1'); 
    ...
    public function socialConnect(Request $request)
    {
        try{
            ...
            $session_oauth_token_secret = $request->session()->get('oauth_token_secret');

            $oauth_credential = new OAuthConsumerCredential($this->api_key,$this->api_secret_key,$session_oauth_token,$session_oauth_token_secret);

            $tripit = new Tripit($oauth_credential);
            $access_token = $tripit->get_access_token();

            $oauthCredentials=new OAuthConsumerCredential($this->api_key,$this->api_secret_key, $access_token['oauth_token'], $access_token['oauth_token_secret']);

            $tripitObject=new Tripit($oauthCredentials);
            $userData=$tripitObject->get_profile();

            $oauthToken = $access_token['oauth_token'];
            $secretOauthToken = $access_token['oauth_token_secret'];
            $userApiID = $userData->Profile->uuid;
            $displayName = $userData->Profile->screen_name;
            $avtar = $userData->Profile->photo_url;
            $city = $userData->Profile->home_city;
            $gender = "";
            ...
            if($displayName != ""){
                if(!PermissionTrait::checkUserApiId($userApiID)){
                    $result = PermissionTrait::storeConnectorUserDetails($this->connectorName,$userApiID,$displayName,$oauthToken,$secretOauthToken,$avtar,$gender,$city);
                    Session::flash('type', $result['type']);
                    Session::flash('msg', $result['message']);
                }else{
