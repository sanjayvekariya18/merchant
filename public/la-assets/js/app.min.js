...
$(document).ready(function() {
    ...
    jQuery.validator.addMethod("data-rule-unique", function(value, element) {
        ...
        if(value != '' && bsurl != "") {
            $.ajax({
				url: bsurl+"/check_unique_val/"+field_id,
				type:"POST",
				async: false,
				data:{
                    'field_value': value,
...
$(document).ready(function() {
    ...
    function getLI(upload) {
        ...
        if($.inArray(upload.extension, ["jpg", "jpeg", "png", "gif", "bmp"]) > -1) {
            image = '<img src="'+upload.path+'?s=130">';
        } else {
    ...
    function loadFMFiles() {
        ...
        $.ajax({
            dataType: 'json',
            url: $('body').attr("bsurl")+"/uploaded_files",
    ...
    $("body").on("click", ".fm_file_sel", function() {
        ...
        if(type == "image") {
            ...
            $hinput.next("a").next(".uploaded_image").children("img").attr("src", upload.path+"?s=150");
        } else if(type == "file") {
            $hinput = $("input[name="+$("#image_selecter_origin").val()+"]");
            $hinput.val(upload.id);
            ...
            $hinput.next("a").next(".uploaded_file").attr("href", upload.path);
        } else if(type == "files") {
            $hinput = $("input[name="+$("#image_selecter_origin").val()+"]");
            ...
            if(upload.extension == "jpg" || upload.extension == "png" || upload.extension == "gif" || upload.extension == "jpeg") {
                fileImage = "<img src='"+upload.path+"?s=90' width='100px' >";
            } else {
                fileImage = "<i class='fa fa-file-o'></i>";
            }
            $hinput.next("div.uploaded_files").append("<a class='uploaded_file2' upload_id='"+upload.id+"' target='_blank' href='"+upload.path+"'>"+fileImage+"<i title='Remove File' class='fa fa-times'></i></a>");
        }
        $("#fm").modal('hide');
    });
});