 ...
 */
    ...
    function sohyperTree(checkParent, treeUrl, treeName, resultId,resultText, isMapping, autoCompleteName, setValueId, isRadio, emptyMessage='') {
        ...
        if (checkParent == false) {
            treeCheckChildren = false;
            if (isRadio == true) {
                treeTemplate = "# if(!item.hasChildren){# <input type='hidden' id='#=item.id#' parent_id='#=item.parent_id#' d_text='#=item.text#'/> <input type='radio' class='radioJsonTree' name_a='#= item.text #' id_a='#= item.id #' name='c_"+treeName+"' value='#=item.text#' data-bind='checked:jsontree' />#}else{# <input type='hidden' id='#=item.id#' parent_id='#=item.parent_id#' d_text='#=item.text#'/> #}#";
            }
            if (isRadio == false) {
        ...
        jQuery.getJSON(treeUrl, function (JsonReturnData) {
            jQuery("#" + treeName).kendoTreeView({
                ...
                dataBound: function () {
                    ...
                    if (!this.dataSource.data().length) {
                        this.element.append("<p class='no-items'><b>"+emptyMessage+".</b></p>");
                    } else {
            ...
            if (isRadio == false) {
                jQuery("#" + treeName).data("kendoTreeView").dataSource.bind("change", function () {
                    ...
                    jQuery("#" + resultId).val(displayMessage);
    ...
    function onBindData(treeName, autoCompleteName, setValueId, isMapping) {
        jQuery('input[type=checkbox]').click(function (eventData) {
            ...
            if (jQuery(this).is(':checked')) {
                jQuery("input[type='checkbox'][id_a='" + idElement + "']").closest("div").find("span:last").addClass("k-state-selected");
                if (isMapping === true) {
                    setTimeout(function () {
                ...
                jQuery("input[type='checkbox'][id_a='" + idElement + "']").closest("div").find("span:last").removeClass("k-state-selected");
                if (isMapping === true) {
                    setTimeout(function () {
    ...
    function getTreeExpandWithSelectedNode(idElement, treePath, treeName, autoCompleteName) {
        if ((jQuery("#" + idElement).length) > (constants.CHECK_LENGTH)) {
            ...
            function loopLi() {
                for (var pathLengthCount = constants.CHECK_LENGTH; pathLengthCount < pathLength; pathLengthCount++) {
    ...
    function getRedioTreeExpandWithSelectedNode(idElement, treePath, treeName, autoCompleteName) {
        if ((jQuery("#" + idElement).length) > (constants.CHECK_LENGTH)) {
            setTimeout(function () {
                getRedioNodeClickWithExpand(idElement, treeName, autoCompleteName);
            }, constants.ADD_MULTISELECT);
        } else {
            selectedNodePath = treePath.split('/');
            pathLength = (selectedNodePath.length);
    ...
    function getRedioNodeClickWithExpand(idElement, treeName, autoCompleteName) {

        jQuery("#" + treeName + " input[id_a='" + idElement + "']").attr('checked', true).trigger('click');
        var treeView = jQuery("#" + treeName).data("kendoTreeView");
        treeView.expand(document.getElementById(idElement));
        jQuery('#' + idElement).closest('div').find('span:last').addClass("k-state-selected");
        kendoUiHoverAutoScrolling(idElement, treeName, autoCompleteName);
