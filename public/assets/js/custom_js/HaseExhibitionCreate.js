$(document).ready(function () {
	...
	$("#hase_exhibition_create").bootstrapValidator({
        fields: {
            exhibition_name: {
                validators: {
                    notEmpty: {
            ...
            $('#hase_exhibition_create')
                    .bootstrapValidator('enableFieldValidators', 'exhibition_date_start', !isEmpty);

            // Revalidate the field when user start typing in the password field
            if ($(this).val().length == 1) {
                $('#hase_exhibition_create').bootstrapValidator('validateField', 'exhibition_date_start');
            }
            data.fv.revalidateField('exhibition_date_start');
            ...
            data.fv.revalidateField('exhibition_date_end');
    });

    $('input[name="holiday_hours[0][date]"]').dateDropper({
        format: "Y-m-d",
        dropPrimaryColor: "#428bca"
    });
    $('input[name="holiday_hours[0][open]"]').timeDropper({
        primaryColor: "#428bca",
        setCurrentTime: false,
        format: "HH:mm"
    });
    $('input[name="holiday_hours[0][close]"]').timeDropper({
        primaryColor: "#428bca",
        setCurrentTime: false,
        format: "HH:mm"
    });
