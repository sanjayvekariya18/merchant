            ...
            <div class="panel panel-primary ">
                <div class="panel-heading">
                    <h4 class="panel-title">
                        <i class="fa fa-fw fa-square-o"></i> Event Url Details
                    </h4>
                </div>
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table class ="table table-bordered" id="eventTable">
                            ...
                            <tbody>
                            @foreach($eventUrlLists as $eventUrlList) 
                                <tr>
                                   <?php if($eventUrlList->field_name != '') {?>
                                    <td>{!!$eventUrlList->field_name!!}</td>
                                <?php } else {?>
                                    <td>{!!$eventUrlList->event_url_node_id!!}</td>
                                <?php } ?>
                                    <td><a target="_blank" href=<?php echo "http://".$eventUrlList->event_url ?>>{!!$eventUrlList->event_url!!}</a></td>         
                            </tr>
                            @endforeach           
                            </tbody>
                        </table>
                        <div class="form-group">
                            ...
                            <select name="queue_status" id="queue_status" onchange="updateSearchEventStatus(this.options[selectedIndex].value)" class="form-control select21">
                               <option>Change Status</option>
                                        @foreach($block_status as $block_status_list)
                                        <option value="{{$block_status_list->status}}">{{$block_status_list->status}}</option>
                                        @endforeach
                            </select>  
                            </div>
                        </div><br><br> 
                         <div class="form-group">
                           <label for="pagination_template" class="col-sm-2 control-label">Pagination Template</label>
                                <div class="col-sm-3">
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form id="create_event_url" method = 'POST' action='{!!url("event_url_detail_update")!!}' class="form-horizontal" enctype="multipart/form-data">
                                                                    ...
                                                                    <button type="button" class="btn btn-success" id="saveStyle" onclick="updateEvnetUrl('event_url_list')" >
...
<script type="text/javascript">
    function updateEvnetUrl(event_detail_table){
        ...
        var event_url=$('#event_url').val();
                if(event_url_title == ''){
                    ...
                    $.ajax({
                        type:'GET',
                        data:{event_url_title:event_url_title,event_url:event_url,website_id:website_id,event_detail_table:event_detail_table},
